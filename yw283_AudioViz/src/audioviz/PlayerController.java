...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(df.format((duration.toMillis())/1000) + " s");
        Duration ct = mediaPlayer.getCurrentTime();
        currentText.setText(ct.toString());
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0.0);
        timeSlider.setMax(duration.toMillis());
        ChangeListener<Boolean> lambdaChangeListener = (ObservableValue<? extends Boolean> observable, Boolean oldValue, final Boolean newValue) -> {
            mediaPlayer.pause();
            if(!newValue) {
        ...
        this.timeSlider.valueChangingProperty().addListener(lambdaChangeListener);
    }
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(df.format(ms/1000) + " s");
